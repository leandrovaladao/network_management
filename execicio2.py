...
def get_mean_of_tec_with_more_than_eighteen_thousand_interrupts():
    columns = extract_file_x()
    list_tcp = []
    for index, it in enumerate(columns['sum_intr']):
        if it > 18000:
...
def get_min_memory_utilization_for_cpu_lower_than_twenty_percent():
    columns = extract_file_x()
    min_memory = 100
    for index, it in enumerate(columns['proc']):
        if it < 20:
            min_memory = columns['memused'][index] if columns['memused'][index] < min_memory else min_memory
